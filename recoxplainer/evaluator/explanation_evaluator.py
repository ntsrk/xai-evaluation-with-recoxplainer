...
class ExplanationEvaluator:
    ...
    # def model_fidelity(self, model_recs, explainer_recs):
    #     # Intersect model and explainer recommendations
    #     merged = pd.merge(model_recs, explainer_recs, on=['userId', 'itemId'])
    #     counts = merged.groupby('userId').size().reindex(range(self.num_users), fill_value=0)
    #     fidelity = (counts / self.top_n).mean()
    #     return fidelity